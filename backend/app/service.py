...
async def _run_dynamic_turn_stream(
    ...
    response_text = ""
    async for token in ollama_chat_completion_stream(
        messages,
        tools=[ctx.tool_definition],
        temperature=0.1,
    ):
        response_text += token
    ...
    if not final_text:
        raise HTTPException(status_code=502, detail="Ollama returned an empty streaming response.")
